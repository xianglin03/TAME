...
OUTPUT_PREFIX = "mnar_task"

# Requested feature-level missing ratio for MNAR evaluation.
MISSING_RATES = [0.1, 0.3, 0.5]
# Strength of dependence between missing mask and the field itself.
MNAR_ALPHA = 1.5
...
def apply_mnar_missing(
    df: pd.DataFrame,
    label_col: str,
    missing_rate: float,
    rng: np.random.Generator,
    alpha: float = MNAR_ALPHA,
    ...
    for target_col in feature_cols:
        base_score = _series_to_score(reference[target_col])
    ...
    for dataname in DATASET_LIST:
        ...
        for missing_rate in eval_missing_rates:
            for setting_idx, (setting_name, train_df) in enumerate(settings.items()):
                ...
                train_masked_df, actual_rate_train = apply_mnar_missing(
                    df=train_df,
                    label_col=label_col,
                ...
                test_masked_df, actual_rate_test = apply_mnar_missing(
                    df=real_test_df,
                    label_col=label_col,
                    missing_rate=missing_rate,
                    rng=test_rng,
