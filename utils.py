...
def execute_function(method, mode):
    ...
    mode_name = 'main' if requested_mode == 'train' else 'sample'
    print('mode', requested_mode)
    print('method', method)
    ...
    if method == 'vae':
        ...
        if requested_mode == 'train_sgd':
            module_name = "baselines.tabddpm.main_train_sgd"
        elif requested_mode == 'train':
            module_name = "baselines.tabddpm.main_train"
        else:
            module_name = "baselines.tabddpm.main_sample"
    else:
        module_name = f"baselines.{method}.{mode_name}"
...
def get_args():
    ...
    parser.add_argument('--eval_flag', type=bool, default=True, help='Whether to evaluate the model.')
    
    parser.add_argument('--dataname', type=str, default='adult', help='Name of dataset.')
    parser.add_argument('--mode', type=str, default='train', help='Mode: train, train_sgd, or sample.')
    parser.add_argument('--method', type=str, default='tabsyn', help='Method: tabsyn or baseline.')
    parser.add_argument('--gpu', type=int, default=4, help='GPU index.') # TODO change the GPU index
    ...
    # configs for DP-SGD training (tabddpm train_sgd)
    parser.add_argument('--dp_mode_num', type=int, default=None, choices=[1, 4, 8, 16, 32], help='DP mode preset for tabddpm DP-SGD. 8 preserves the current default training setup.')
    parser.add_argument('--dp_noise_multiplier', type=float, default=1.0, help='Noise multiplier for DP-SGD.')
    parser.add_argument('--dp_max_grad_norm', type=float, default=1.0, help='Per-sample gradient clipping norm for DP-SGD.')
    parser.add_argument('--dp_delta', type=float, default=1e-5, help='Target delta for privacy accountant.')
    parser.add_argument('--dp_secure_mode', action='store_true', default=False, help='Use secure RNG in Opacus privacy engine.')
    parser.add_argument('--dp_physical_batch_size', type=int, default=1024, help='Physical batch size used by Opacus BatchMemoryManager. Set <=0 to disable splitting.')
    parser.add_argument('--dp_grad_sample_mode', type=str, default='hooks', choices=['ghost', 'hooks'], help='Opacus grad sample mode. hooks is the safer default; ghost can be faster when supported.')
    parser.add_argument('--dp_log_every', type=int, default=50, help='How often to aggregate and record training loss.')
    parser.add_argument('--dp_print_every', type=int, default=50, help='How often to print training progress.')
    parser.add_argument('--dp_epsilon_every', type=int, default=1000, help='How often to query the privacy accountant for epsilon.')
    parser.add_argument('--dp_target_epochs', type=int, default=800, help='Upper bound on effective training epochs for DP-SGD.')
    parser.add_argument('--dp_max_updates', type=int, default=3000, help='Hard cap on optimizer updates for DP-SGD.')
    parser.add_argument('--dp_min_updates', type=int, default=1000, help='Minimum optimizer updates before DP-SGD can finish.')
    parser.add_argument('--dp_warmup_ratio', type=float, default=0.05, help='Warmup ratio for the DP learning-rate schedule.')
    parser.add_argument('--dp_min_lr_ratio', type=float, default=0.05, help='Final learning-rate ratio for cosine decay in DP-SGD.')
    parser.add_argument('--dp_ema_decay', type=float, default=0.999, help='EMA decay used to smooth DP model weights.')
    parser.add_argument('--dp_metric_smoothing', type=float, default=0.98, help='EMA smoothing applied to the logged training loss for early stopping.')
    parser.add_argument('--dp_early_stop_patience', type=int, default=400, help='Stop if the smoothed DP loss does not improve for this many updates.')
    parser.add_argument('--dp_early_stop_min_steps', type=int, default=800, help='Do not trigger DP early stopping before this many updates.')
    parser.add_argument('--dp_improvement_tol', type=float, default=1e-4, help='Minimum smoothed-loss improvement required to refresh the DP best checkpoint.')
    parser.add_argument('--dp_steps', type=int, default=3000, help='Total training steps for TabSyn DP-SGD.')
